...
Every task MUST have files: and verify:. Task verify: must be FILE-SCOPED (runner filter) — global build/suite belongs to [phase-verify:] and the orchestrator. `agent:` is optional specialist routing (agent-routing.md) — only types that exist this session; `agent: orchestrator` = runs inline, never fans out (scaffold/manifest/shared-config tasks). Orchestrator checks boxes only after ITS OWN verification (Hard rule 2).>

## P1 — Scaffold   [phase-verify: `<build command>`]
- [ ] P1.1 <init app skeleton, tooling, test runner> · files: / · agent: orchestrator · verify: `<build>`
